...
/// Semantic tags storage - maps unit IDs to their tags
...
impl SemanticTags {
    ...
    /// Add a tag to a unit
    pub fn add_tag(&mut self, unit_id: &str, tag: &str) -> anyhow::Result<()> {
        // Validate tag format (namespace:value)
        if !tag.contains(':') {
            anyhow::bail!(
                "Invalid tag format '{}'. Tags must be namespace:value (e.g., domain:auth)",
                tag
            );
        }
        let tags = self.tags.entry(unit_id.to_string()).or_default();
    ...
    pub fn remove_tag(&mut self, unit_id: &str, tag: &str) {
        if let Some(tags) = self.tags.get_mut(unit_id) {
            tags.retain(|t| t != tag);
            if tags.is_empty() {
    ...
    pub fn clear_tags(&mut self, unit_id: &str) {
        self.tags.remove(unit_id);
    }
}
