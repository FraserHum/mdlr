...
/// and semantic resolution (when a `ResolutionContext` is provided) to
...
fn build_name_index(units: &[Unit]) -> HashMap<String, Vec<String>> {
    ...
    for unit in units {
        ...
        let Some(idx) = unit.id.find("::") else { continue };
        ...
        // Index impl methods
        index_impl_methods(&mut name_to_ids, local, &unit.id);
...
/// Index methods in impl blocks by method name and Type::method form.
fn index_impl_methods(
    ...
    let Some(method_idx) = impl_part.rfind("::") else { return };
...
fn index_short_name(
    ...
    let Some(last_idx) = local.rfind("::") else { return };
