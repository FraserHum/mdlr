...
use clap::Parser;
use mdlr::cache::{
    CacheStore, FileCacheEntry, get_file_metadata, now_timestamp,
};
use mdlr::cli::{Cli, Command, MetricsCommand, OutputFormat};
use mdlr::config;
use mdlr::extract::{Extractor, extractor_for_path};
use mdlr::graph::{Graph, Unit, build as build_graph};
use mdlr::json_output::{
    build_bucketed_json, build_complexity_json, build_fan_metrics_json,
    build_file_loc_json, build_impl_json, build_semantic_tags_json,
};
use mdlr::metrics::{
    BucketedMetrics, ComplexityMetrics, FileLocMetrics, ImplMetrics,
    MetricsBundle, TagMetrics, collect_metric_rows,
};
use mdlr::resolve::{CargoWorkspace, ResolutionContext};
use mdlr::symbol_commands::{handle_get, handle_ls};
use mdlr::tag_commands::{
...
fn get_metric_descriptions() -> Vec<(&'static str, &'static str)> {
    vec![
        ...
        (
            "methods_per_impl",
            "Number of methods in an impl block. High values may indicate a type with too many responsibilities.",
        ),
        (
            "traits_per_type",
            "Number of traits implemented by a type. High values may indicate a versatile type or one trying to do too much.",
        ),
        (
            "lcom",
            "Lack of Cohesion of Methods. High values indicate methods don't share state, suggesting the impl could be split.",
        ),
        (
            "file_loc",
...
struct ComputedMetrics {
    graph: Graph,
    structural: mdlr::metrics::StructuralMetrics,
    complexity: ComplexityMetrics,
    impl_metrics: ImplMetrics,
...
fn process_uncached_file(
    ...
    match extract_file(file_path, extractor.as_ref(), resolution_ctx) {
        Ok(units) => {
            result.extracted_count += 1;
            if save {
                result.entries_to_save.push(build_cache_entry(
...
fn compute_all_metrics(
    ...
    let graph = build_graph(units, resolution_ctx);
    let structural = mdlr::metrics::compute(&graph);
...
fn format_text_output(
    ...
    let bundle = MetricsBundle {
        ...
        impl_metrics: &computed.impl_metrics,
...
fn format_json_output(
    ...
    let bucketed = BucketedMetrics::from_metrics(&computed.structural, config);
    ...
    let output = serde_json::json!({
        ...
        "metrics": {
            ...
            "impl": build_impl_json(&computed.impl_metrics),
...
fn build_symbol_json(
    ...
    {
        let bucket =
            config.thresholds.methods_per_impl.evaluate(*value as f64);
        metrics.insert(
            "methods_per_impl".to_string(),
            serde_json::json!({ "value": value, "bucket": bucket.to_string() }),
        );
    }
    ...
    if let Some((_, value)) = computed
        ...
        .find(|(name, _)| name == symbol_id)
    {
        let bucket = config.thresholds.traits_per_type.evaluate(*value as f64);
        metrics.insert(
            "traits_per_type".to_string(),
            serde_json::json!({ "value": value, "bucket": bucket.to_string() }),
        );
    }
